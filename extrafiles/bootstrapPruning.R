...
bootstrapPruning <- function(intree,dipolarmodel,bts = c(),splitrule,alpha) {
...
for (i in 1:bts[1]){
btsamps[i,]<- sort(sample(strtoi(rownames(intree$data)),floor(bts[2]*nrow(alldata)),replace=F)) 
}
...
logrank.compute <- function(traintree){
  logrank.node.compute = function(trainnode){
    subnames = rownames(trainnode$testX)
...
bestgeomean = geomeans[which.max(statformax(alpha))] 
...
FinalPrunedtree$Do(function(node) node$KMest=KMcompute(node), filterFun = isLeaf)
return(FinalPrunedtree)
}
